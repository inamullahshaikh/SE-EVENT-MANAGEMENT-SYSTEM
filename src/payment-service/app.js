...
// Ensure that error messages are structured as expected in the tests
app.post("/create", async (req, res) => {
  try {
    ...
    if (!cardNumber || !cardHolderName || !dateOfIssue || !dateOfExpiry || !cvc || !attendee) {
      return res.status(500).send({
        message: "Some required fields are missing", // Use message as expected in the tests
      });
    }
    ...
    if (existingPayment) {
      return res.status(400).send({
        message: "This card is already added by the user.", // Use message as expected in the tests
      });
    }
    ...
    res.status(500).send({ message: "DB error" }); // Use message for consistency
  }
});
...
// At the bottom of your payment service file
module.exports = app;
